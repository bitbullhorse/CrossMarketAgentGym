...
def test_ecb_transform_and_matrix_audit(
    ...
    assert matrix.matrix_id == "phase12-run-matrix-v6"
    assert all(task.run_id.startswith("p12v4m6-") for task in matrix.tasks)
    ...
    superseded_v5 = build_run_matrix(
        protocol,
        protocol_sha256="a" * 64,
        code_commit="b" * 40,
        matrix_revision=5,
    )
    assert superseded_v5.matrix_id == "phase12-run-matrix-v5"
    assert all(
        task.run_id.startswith("p12v4m5-")
        for task in superseded_v5.tasks
    )
