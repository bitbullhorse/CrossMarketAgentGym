...
## DL-0097 — Accept external Phase 12 review disposition without fabricating evidence

- Decision: accept the project owner's 2026-07-29 confirmation that the independent audit is
  complete and P0/P1 are zero. Record only the resulting phase disposition; do not create a
  reviewer identity, signature, review narrative, or completed review form.
- Reason: the owner explicitly waived repository storage of the independent review. This closes
  the gate while preserving the distinction between a supplied external conclusion and evidence
  generated by this software agent.

## DL-0098 — Freeze large Phase 12 payloads by content identity

- Decision: benchmark-v1 embeds compact metrics, statistical outputs, Agent Replay, and HPO
  audit logs. Large trades, weights, models, and checkpoints remain in the immutable formal
  result root and are represented by run ID, relative path, byte size, and SHA-256.
- Reason: copying approximately 230 GB into a source benchmark is impractical and creates a
  second mutable payload. Content-addressed indexes retain exact provenance and allow external
  retrieval and verification without weakening the frozen result identity.

## DL-0099 — Add Phase 13 CLI commands without widening the stable Python API

- Decision: add `cmag benchmark build`, `cmag benchmark verify`,
  `cmag paper export-tables`, and `cmag paper export-figures` to the reviewed CLI inventory.
  Keep the implementation package internal and add no top-level `__all__` exports.
- Reason: the Phase 13 protocol explicitly requires these automation commands. An additive,
  audited CLI extension satisfies that protocol while preserving all 251 previously frozen
  Python API records and every frozen configuration and artifact Schema.

## DL-0100 — Accept the externally confirmed Benchmark v1 review disposition

- Decision: close Phase 13 from the project owner's 2026-07-29 confirmation that a non-author
  review approved Benchmark v1 with P0/P1 equal to zero. Persist only the disposition, not an
  invented reviewer identity, signature, or review narrative.
- Reason: the external review satisfies the final Phase 13 gate. Keeping the supplied conclusion
  distinct from repository-generated evidence preserves honest provenance.

## DL-0101 — Bind stable v1.0.0 to the actual accepted formal revisions

- Decision: map stable v1.0.0 to Benchmark v1, Dataset Manifest v3, protocol-v4 and formal
  experiment commit `6f03d3da3ed6ecbe918c5a7f9aa35cb9abfb2b83`. Treat the Phase 14 report's
  dataset-manifest-v1/protocol-v1 labels as pre-experiment placeholders and retain the canonical
  package name `crossmarket-agent-gym`.
- Reason: v1 and later v2 inputs were superseded before eligible formal execution because of
  leakage and data-semantic defects. Renaming accepted v3/v4 evidence or reverting to an older
  input would break Benchmark hashes and scientific provenance.

## DL-0102 — Separate local release readiness from irreversible publication

- Decision: provide dry-run archive, package, container, documentation and public-verification
  automation. Do not create `v1.0.0` from a dirty tree or claim Phase 14 completion before PyPI,
  GHCR, Pages, GitHub Release and a real DOI are independently accessible.
